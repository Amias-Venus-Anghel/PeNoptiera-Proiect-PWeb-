...
public class ShoppingCartController : AuthorizedController
{
    ...
    public async Task<ActionResult<RequestResponse<PagedResponse<ProductDTO>>>> GetProductsInCartOfUser([FromQuery] PaginationSearchQueryParams pagination) // The FromQuery attribute will bind the parameters matching the names of
                                                                                                                                                      // the PaginationSearchQueryParams properties to the object in the method parameter.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            this.FromServiceResponse(await _shoppingCartService.GetProductsInCartOfUser(pagination, currentUser.Result)) :
